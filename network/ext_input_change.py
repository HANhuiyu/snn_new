...
class ExternalInputReader():
	...
	print("the length of the eventlist is ",len(eventList))
        ...
        for x in range(0,dim_x):
            for y in range(0, dim_y):
                self.retinaLeft.append(retinaL[x][y])

        for x in range(0,dim_x):
            for y in range(0,dim_y):
                self.retinaRight.append(retinaR[x][y])
        #print("the nouveau list of left is ", self.retinaLeft)
        #print("nouveau list of right is ",self.retinaRight)
